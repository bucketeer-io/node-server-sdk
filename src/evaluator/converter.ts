...
import { RolloutStrategy as ProtoRolloutStrategy, FixedStrategy as ProtoFixedStrategy } from '@bucketeer/evaluation/lib/proto/feature/strategy_pb';

function mapVariationType(type: string): ProtoFeature.VariationTypeMap[keyof ProtoFeature.VariationTypeMap] {
  switch (type.toUpperCase()) {
    case 'STRING': return ProtoFeature.VariationType.STRING;
    case 'BOOLEAN': return ProtoFeature.VariationType.BOOLEAN;
    case 'NUMBER': return ProtoFeature.VariationType.NUMBER;
    case 'JSON': return ProtoFeature.VariationType.JSON;
    default: return ProtoFeature.VariationType.STRING;
  }
}

function mapOperator(op: string): ProtoClause.OperatorMap[keyof ProtoClause.OperatorMap] {
  switch (op.toUpperCase()) {
    case 'EQUALS': return ProtoClause.Operator.EQUALS;
    case 'IN': return ProtoClause.Operator.IN;
    case 'STARTS_WITH': return ProtoClause.Operator.STARTS_WITH;
    case 'ENDS_WITH': return ProtoClause.Operator.ENDS_WITH;
    case 'SEGMENT': return ProtoClause.Operator.SEGMENT;
    case 'GREATER': return ProtoClause.Operator.GREATER;
    case 'GREATER_OR_EQUAL': return ProtoClause.Operator.GREATER_OR_EQUAL;
    case 'LESS': return ProtoClause.Operator.LESS;
    case 'LESS_OR_EQUAL': return ProtoClause.Operator.LESS_OR_EQUAL;
    case 'BEFORE': return ProtoClause.Operator.BEFORE;
    case 'AFTER': return ProtoClause.Operator.AFTER;
    case 'PARTIALLY_MATCH': return ProtoClause.Operator.PARTIALLY_MATCH;
    default: return ProtoClause.Operator.EQUALS;
  }
}

function mapStrategyType(type: string): ProtoStrategy.TypeMap[keyof ProtoStrategy.TypeMap] {
  switch (type.toUpperCase()) {
    case 'FIXED': return ProtoStrategy.Type.FIXED;
    case 'ROLLOUT': return ProtoStrategy.Type.ROLLOUT;
    default: return ProtoStrategy.Type.FIXED;
  }
}

function mapSegmentUserState(state: string): ProtoSegmentUser.StateMap[keyof ProtoSegmentUser.StateMap] {
  switch (state.toUpperCase()) {
    case 'INCLUDED': return ProtoSegmentUser.State.INCLUDED;
    case 'EXCLUDED': return ProtoSegmentUser.State.EXCLUDED;
    default: return ProtoSegmentUser.State.INCLUDED;
  }
}
...
export function toProtoFeature(feature: Feature): ProtoFeature {
  ...
  if (feature.variations) {
    f.setVariationsList(feature.variations.map((v) => {
  ...
  if (feature.targets) {
    f.setTargetsList(feature.targets.map((t) => {
  ...
  if (feature.rules) {
    f.setRulesList(feature.rules.map((r) => {
      ...
      pr.setClausesList(r.clauses.map((c) => {
  ...
  if (feature.prerequisites) {
    f.setPrerequisitesList(feature.prerequisites.map((p) => {
...
function mapStrategy(s: SDKStrategy): ProtoStrategy {
  ...
  if (s.rolloutStrategy) {
    ...
    pr.setVariationsList(s.rolloutStrategy.variations.map((v) => {
...
export function toProtoSegmentUsers(segmentUsers: SegmentUsers): ProtoSegmentUsers {
  ...
  psu.setUsersList(segmentUsers.users.map((u) => {
