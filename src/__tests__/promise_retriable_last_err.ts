...
import { InvalidStatusError, TimeoutError, AbortError } from '../objects/errors';
...
test.serial('Path A: TimeoutError from fn is replaced by lastHttpError (499)', async (t) => {
  ...
  fn.onCall(1).callsFake(() => {
    controller.abort(new TimeoutError(30_000));
    return Promise.reject(new TimeoutError(30_000));
  });
